...
func LoadClientConfig(content []byte, strict bool) (
	*v1.ClientCommonConfig,
	...
	// In frp v0.69+, Complete() no longer accepts parameters.
	// User field is now set on the ProxyBaseConfig directly from cliCfg.User
	for _, c := range proxyCfgs {
		c.Complete()
	}
	for _, c := range visitorCfgs {
		// VisitorConfigurer Complete() also doesn't accept parameters anymore
		c.Complete()
	}
